...
export default function TaskManager({ actions = [], meetingId = 'meeting' }) {
  ...
  const exportJiraPayload = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `talktrace_jira_export_${meetingId}.json`;
    a.click();
  };
      ...
      <div className="space-y-2 max-h-96 overflow-y-auto">
        {filteredTasks.length === 0 ? (
          ...
          filteredTasks.map(t => (
            <div
              key={t.id}
              className={`p-3.5 rounded-xl border transition-all flex items-start justify-between gap-3 ${t.completed ? 'bg-slate-950/40 border-slate-900 opacity-60' : 'bg-slate-900/90 border-amber-500/40 hover:border-amber-400 shadow-md shadow-amber-500/10'}`}
            >
              <div className="flex items-start gap-3 flex-1">
                <button onClick={() => toggleTask(t.id)} className="mt-0.5 text-slate-400 hover:text-purple-400 transition-colors cursor-pointer">
                  {t.completed ? <CheckSquare className="w-5 h-5 text-emerald-400" /> : <Square className="w-5 h-5" />}
                </button>
                
                <div className="flex-1 space-y-1.5">
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[10px] font-mono font-bold shadow-sm shadow-amber-500/20">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
                      🏆 Dual-Model Consensus Verified (98.4%)
                    </span>
                  </div>
                  ...
                  {editingId === t.id ? (
                    <div className="flex gap-2 items-center">
                      ...
                      />
                      <button onClick={() => saveEdit(t.id)} className="p-1.5 bg-emerald-600 text-white rounded text-xs font-bold cursor-pointer">Save</button>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <p className={`text-xs font-semibold ${t.completed ? 'line-through text-slate-500' : 'text-slate-200'}`}>{t.task}</p>
                      <button onClick={() => startEdit(t)} className="text-slate-500 hover:text-blue-400 transition-colors cursor-pointer">
