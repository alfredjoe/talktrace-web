  ...
  const joinMeeting = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setStatus('active');
        setStatusMessage('Meeting active. Recording in progress...');
  ...
  const renderLibrary = () => {
    ...
    return (
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* Search Bar & Search History Dropdown */}
        <div className="relative mb-6">
          <div className="flex gap-2 items-center">
            ...
            {searchQuery.trim() && (
              <button
                onClick={() => { saveSearchQuery(searchQuery); setShowSearchDropdown(false); }}
                className="px-4 py-3 btn-sleek-primary text-white rounded-xl text-xs font-semibold transition-all shadow-md"
              >
                Search
              </button>
            )}
          ...
          {/* Dropdown Menu for User-Isolated Search History */}
          {showSearchDropdown && searchHistory.length > 0 && (
            <div
              className="absolute top-full left-0 right-0 mt-2 sleek-card rounded-xl shadow-2xl z-30 p-2 text-slate-300 border border-slate-700/80 backdrop-blur-xl animate-in fade-in slide-in-from-top-2"
            >
              <div className="flex justify-between items-center px-3 py-2 border-b border-slate-800 text-xs font-semibold text-slate-400">
                <span className="flex items-center gap-1.5 uppercase tracking-wider text-[10px]">
                  <History className="w-3.5 h-3.5 text-blue-400" /> Recent Searches (Private)
                </span>
                <button
              ...
              <div className="max-h-48 overflow-y-auto py-1">
                {searchHistory.map((item) => (
                  <div
                    ...
                    className="flex items-center justify-between px-3 py-2 hover:bg-slate-800/80 rounded-lg cursor-pointer text-sm group transition-colors"
                  >
                    <span className="flex items-center gap-2 truncate text-slate-300 group-hover:text-blue-400">
                      <Clock className="w-3.5 h-3.5 text-slate-500" />
                      {item.query}
                    </span>
                    <button
                      onClick={(e) => deleteSearchHistoryItem(e, item.id)}
                      className="p-1 hover:bg-slate-700/80 text-slate-500 hover:text-red-400 rounded transition-colors"
              ...
              <div key={m.meeting_id}
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2 text-sm text-slate-400">
                    ...
                    {m.language && (
                      <span className="ml-2 px-1.5 py-0.5 bg-slate-900 text-[10px] rounded border border-slate-700 uppercase">
                        🌐 {m.language}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        disabled={status !== 'idle'}
                        className="w-full bg-slate-900/90 border border-slate-700/80 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-xl py-3.5 pl-12 pr-4 text-white placeholder-slate-500 transition-all outline-none disabled:opacity-50 text-sm font-medium"
                      />
                    </div>
                    <div className="flex gap-4">
                      {status === 'idle' ? (
                        ...
                        <button
                          onClick={reset}
                          className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold py-3.5 rounded-xl transition-all border border-slate-700 shadow-sm text-sm"
                        >
                          Reset / New Meeting
                        </button>
                      )}
